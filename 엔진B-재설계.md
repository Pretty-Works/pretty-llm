...
| `project.followup` | **신설** | 회의록 후속조치 ↔ 할 일 대조. "회의만 하고 넘어간 항목"을 잡는다 |
| `me.my_week` | **신설** | 본인 스코프 전용 |

### `project.followup` — 얇아진 자리를 메운 축

`skill_fit` 이 개인 평가를 빼면서 얇아졌다. 그 자리를 **개인정보와 무관하면서 실무적으로
쓸모 있는 분석**으로 메웠다.

회의록(`content`·`follow_up`)과 프로젝트 할 일을 대조해서, 회의에서 하기로 했는데
할 일로 등록조차 안 된 항목(`UNTRACKED`)과 등록됐지만 마감이 지난 항목(`STALLED`)을 찾는다.
프로젝트가 새는 지점인데 지금까지 아무 축도 안 봤다.

근거가 둘 다 프로젝트 화면에 보이는 문서라 권한 경계 문제도 없다.

> ⚠️ 마일스톤·회의록을 컨텍스트에 실어놓고 정작 워커의 `context_sections` 에 안 넣어
> 데이터가 죽어 있었다. 전 워커에 연결했다 — 데이터를 붙이는 것과 워커가 보는 것은 별개다.
...
1차 재편 후 축을 다시 놓고 보니 나눌 이유가 없는 것과 남의 일을 하는 것이 있었다.

### `skill_fit` + `workload` → `staffing` (합병)
...
| | 전 | 후 |
|---|---|---|
| workload | "B에게 넘길 수 있음" | **"이 일은 FE 성격이고 이 프로젝트 FE 는 B다. 그래서 B가 받을 수 있다"** |
| skill_fit | "B는 FE 역할" | (합쳐짐) |

확신도 상한(0.75)은 **역할 판단이 들어간 출력에만** 건다. 가용성 숫자는 코드가 계산한
값이라 깎을 이유가 없다 — 합병 전 `workload` 에 상한이 없던 것과 같은 이유다.

### `risk` — 조사하지 않는 종합 축으로

`RiskCategory` 가 `schedule | resource | scope | quality | cost | external` 이었다.
`schedule` 은 우선순위 축이, `resource` 는 인력 배치 축이, `cost` 는 비용 축이 **이미 보는
것**인데 risk 가 같은 데이터를 병렬로 받아 독립적으로 다시 판단했다. 결과가 실제로 나타났다.

```
· project/risk/project/priority: 일정 위험과 우선순위 조정 필요성   ← 가짜 상충
[error] UNKNOWN_SUBJECT (risk) ... todo:101, todo:102, todo:106 가 데이터에 없다
```

진짜 상충이 아니라 같은 사실을 두 축이 따로 본 것이고, `UNKNOWN_SUBJECT` 가 risk 에서만
나온 것도 자기 근거를 새로 만들어야 했기 때문이다.

**조치**: 내부 조회 도구 전부 제거(`tools=()`), `cost` 범주 삭제, 프롬프트를 "새 사실을
찾지 않는다 / **둘 이상의 사실이 겹치는 것만** 위험으로 올린다"로 재작성.
gmail 읽기 도구만 남겼다 — 외주사 지연 통보처럼 내부 데이터에 아예 없는 신호라서다.

### `priority` — 근거 없는 필드 제거

`blocks`(할 일 간 의존 관계)를 없앴다. **DB 에 의존 관계 컬럼이 없어서** 모델이 제목만 보고
지어내던 필드였다. `ranked` 는 "전체"가 아니라 "컨텍스트 범위"임을 명시하고 `scope_note` 를
추가했다 — `/tasks` 가 주 단위라 전체는 애초에 볼 수 없다.

### 결과

축 6개 → **5개** (`priority` · `risk` · `cost` · `followup` / `staffing`), 그리고 `me.my_week`.
재설계 후 첫 전체 실행에서 **검증 위반 0건**이 나왔다.
