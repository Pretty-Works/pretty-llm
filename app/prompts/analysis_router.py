...
2. domains : 이 질문에 실제로 필요한 도메인만 고른다.
   ...
   ★ 주어가 "나"면 project 나 hcm 을 절대 얹지 마라 — 대상 프로젝트를 질문·화면
     어디서도 특정할 수 없는데 project/hcm 을 붙이면, 시스템이 "요청자가 참여
     중인 프로젝트"를 임의로 끌어와 그 프로젝트의 다른 참여자 정보가 답변에
     섞여 들어간다(실제 신원 혼동 사고 이력 — 서강우가 물었는데 강나은 정보가
     나온 사고). "나"가 주어인 질문은 me 아니면 vacation 둘 중 하나로만 끝난다.
   ★ me vs vacation 은 아래처럼 **내용으로만** 가른다. 문장에 "프로젝트에
     지장없이"류 문구가 있는지는 절대 기준으로 쓰지 마라 — 표현 하나로 갈리는
     기준은 LLM 이 안정적으로 못 지킨다(실제로 이 기준 때문에 라우팅이
     project 로 새어나간 사고가 있었다):
       · 휴가·연차 이야기가 조금이라도 나오고, 그 시점·가부(언제 쓸지, 써도
         되는지, 날짜 추천)를 묻는 질문 → **무조건 vacation**. "휴가 언제
         쓸까", "다음 주에 하루 쉬어도 될까", "연차 언제 쓰는 게 좋을까"
         전부 vacation 이다. "지장없이"가 있든 없든 상관없다.
       · 휴가·연차 언급이 없거나, 있어도 여러 할 일 중 하나로만 섞여 나오고
         질문의 핵심이 "전체적으로 뭐부터 할지" 우선순위를 정하는 것이면 me
         다(예: "할 일도 많고 휴가도 신청해야 하고... 뭐부터 하지" — 휴가
         자체의 시점을 묻는 게 아니라 할 일 순서를 묻는 질문이라 me).
   ★ hcm 은 **대상 프로젝트가 있을 때만** 고른다. 참여자 명단이 없으면 아무것도 못 본다.

   ★ 매우 중요: 도메인을 고르면 그 도메인의 워커가 **전부 병렬로 실행**된다.
     (project 를 고르면 우선순위·위험·비용·회의 후속조치가 모두 돈다.)
...
FEW_SHOTS = [
    ...
    {
        # ★ 2026-08-13 추가 — 실제 신원 혼동 사고("서강우가 물었는데 강나은이 나옴")를
        # 그대로 재현한 문구다. "프로젝트에 지장없이"류 명시적 문구가 하나도 없다는
        # 점이 핵심 — 그 문구 유무로 me/vacation 을 가르면 이 질문처럼 문구가 빠진
        # 변형이 project 로 새어나간다. 대상 프로젝트가 전혀 특정되지 않았는데도
        # domains 에 project/hcm 을 얹지 않는 것도 이 예시가 함께 보여준다.
        "query": "다음주 휴가 언제 쓰는게 좋을까",
        "ui": "screen=home",
        "answer": {
            "mode": "analysis",
            "domains": ["vacation"],
            "focus": [],
            "objective": "다음 주 중 안전하게 휴가를 쓸 수 있는 날짜를 확인한다",
            "entities": {
                "date_from": "2026-08-17",
                "date_to": "2026-08-23",
            },
            "constraints": [],
            "reasoning": "[오늘]이 2026-08-12(수)라고 가정하면 이번 주 월요일은 08-10이고 '다음 주'는 08-17~08-23이다. 휴가 시점을 묻는 자기참조 질문은 '프로젝트에 지장없이'라는 문구가 없어도 무조건 vacation이다 — 그 문구 유무로 me/vacation을 가르지 않는다. 대상 프로젝트를 특정할 근거가 전혀 없으므로 project/hcm은 얹지 않는다 — 얹으면 요청자가 참여 중인 프로젝트를 시스템이 임의로 끌어와 다른 참여자 정보가 섞여 들어갈 위험이 있다.",
            "confidence": 0.88,
        },
    },
    {
