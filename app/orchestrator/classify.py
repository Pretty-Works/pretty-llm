...
meeting(회의록 — **이미 한 회의의 기록**) · vacation(연차·휴가)
project(프로젝트 자체 — **마일스톤·예산·팀원도 전부 여기**) · task(할일·업무)
schedule(일정·캘린더 — **앞으로의 약속·미팅 잡기**) · expense(경비·비용)
...
⚠️ **"마일스톤"이라는 말이 나오면 무조건 project 다** — 절대 task 가 아니다.
  마일스톤은 프로젝트의 단계 목표(요구사항 확정·베타 오픈 …)이고, 할 일과는
  다른 데이터다. 마일스톤 도구(milestone_list·milestone_toggle_status)는
  project 담당 에이전트만 갖고 있어서, task 로 보내면 그 에이전트는 마일스톤을
  못 만지고 **엉뚱한 할 일을 대신 완료 처리하려 든다**.
  (실측 사고: "요구사항 확정 마일스톤 완료 처리해줘" → task 로 분류 →
   무관한 할 일 #2393 을 완료 처리하겠다는 승인 카드가 떴다)
  "요구사항 확정 마일스톤 완료 처리해줘"     → ["project"]
  "마일스톤 뭐 남았어?" / "다음 마일스톤 언제야?" → ["project"]
...
- ⚠️ "주간보고"·"위클리 리포트"·"이번주 업무 정리해줘"처럼 한 주의 업무를 종합해
  달라는 요청은 여러 도메인(할일·일정·회의)의 자료를 참고하긴 해도, 도메인은
  ["task"] 하나만 넣으세요 — task 담당 에이전트가 그 안에서 필요한 자료를 전부
  모아 보고서 하나로 종합하도록 만들어져 있습니다. 여기서 ["task","schedule",
  "meeting"] 처럼 여러 개로 쪼개면 도메인마다 따로따로 답한 짧은 문장들이 이어
  붙을 뿐, 하나의 보고서로 종합되지 않습니다. route 는 engine_a.
  예: "이번주 주간보고 작성해줘" → route: engine_a, domains: ["task"]"""
